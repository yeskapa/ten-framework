...
class BedrockV2VConfig(BaseConfig):
    ...
    prompt: str = "You are a friendly assistant. The user and you will engage in a spoken dialog " \
...
class SimpleNovaSonic:
    ...
    async def start_session(self, tools: List[LLMToolMetadata]):
        ...
        self.ten_env.log_info(f"start_session...., prompt_name: {self.prompt_name}, content_name: {self.content_name} ")
        if not self.client:
                    ...
                    if 'event' in json_data:
                        if 'contentStart' in json_data['event']:
                            ...
                            if content_type == 'TOOL' and self.role == 'TOOL':
                                self.ten_env.log_info(f"Tool content start detected: {content_start}")
                            ...
                            if not self.is_interrupted:
                                if (self.role == "ASSISTANT" and self.display_assistant_text):
                                    ...
                                    await self.text_queue.put(("assistant", text, False))
                                elif self.role == "USER":
                                    print(f"User: {text}")
                            ...
                            self.ten_env.log_info(f"Tool use received: {tool_name} with ID {tool_use_id}")
                            await self.tool_call_queue.put((tool_use_id, tool_name, tool_content))
                            ...
                            if stop_reason == "INTERRUPTED":
                                self.ten_env.log_info("INTERRUPTED signal detected, stopping output")
                                self.is_interrupted = True
                                flush_cmd = Cmd.create("flush")
                                await self.ten_env.send_cmd(flush_cmd)
                            elif stop_reason == "END_TURN" or stop_reason == "INTERRUPTED":
                                if self.role is not None:
                                    await self.text_queue.put((self.role, "", True))
                                    
                            self.ten_env.log_info(f"Content end detected: {content_end},  role: {self.role}")
        except Exception as e:
            traceback.print_exc()
...
class BedrockV2VExtension(AsyncLLMBaseExtension):
    ...
    async def on_stop(self, ten_env: AsyncTenEnv) -> None:
        ...
        if self.nova_client:
            try:
                ...
                if self.nova_client.stream and hasattr(self.nova_client.stream, 'input_stream'):
                    try:
    ...
    async def _handle_tool_call(self, ten_env: AsyncTenEnv, tool_call_id: str, name: str, arguments: str) -> None:
        ...
        if result.get_status_code() == StatusCode.OK:
            ...
            await self.nova_client.send_tool_response(tool_call_id, {"error": "Tool call failed"})
        ...
        try:
            ...
            if self.is_active and self.nova_client:
                ...
                try:
                    ...
                    ten_env.log_error(f"Error processing audio responses: {audio_resp_error}")
                ...
                try:
                    ...
                    while not self.nova_client.tool_call_queue.empty():
                        tool_call_id, tool_name, tool_arguments = await self.nova_client.tool_call_queue.get()
                        await self._handle_tool_call(ten_env, tool_call_id, tool_name, tool_arguments)
            ...
            if self.nova_client:
                ten_env.log_info("Attempting to reset session due to audio frame processing error")
                try:
                ...
                try:
                    ...
                    await self.nova_client.start_audio_input()
                    ten_env.log_info("Successfully restarted session after audio frame error")
                except Exception as restart_error:
                    ten_env.log_error(f"Failed to restart session after audio frame error: {restart_error}")
            
    async def _send_transcript(self, ten_env: AsyncTenEnv, content: str, role: str, is_final: bool) -> None:
        """Send text transcript to the client."""
        ten_env.log_debug(f"Sending transcript: {content}, role: {role}, is_final: {is_final}")
        ...
        try:
            if role == "assistant" and not is_final:
                ...
                for s in sentences:
                    ...
                    ten_env.log_info(f"send transcript text [{s}] stream_id {stream_id} is_final {is_final} role {role}")
                ...
                d.set_property_string("role", role)
                d.set_property_int("stream_id", stream_id)
                ten_env.log_info(f"send transcript text [{content}] stream_id {stream_id} is_final {is_final} role {role}")
                await ten_env.send_data(d)
        except Exception as e:
            ten_env.log_error(f"Error send text data {role}: {content} {is_final} {e}")

    async def _send_audio_response(self, ten_env: AsyncTenEnv, audio_data: bytes) -> None:
        ...
        if self.nova_client and self.nova_client.is_active:
            try:
                ...
                try:
                    ...
                    if self.nova_client.stream and hasattr(self.nova_client.stream, 'input_stream'):
                        try:
                            ...
                            ten_env.log_error(f"Error closing stream during flush: {close_error}")
                ...
                try:
                    ...
                    ten_env.log_error(f"Failed to restart session after flush error: {restart_error}")
